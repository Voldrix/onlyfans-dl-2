...
from config import TELEGRAM_BOT_TOKEN, TELEGRAM_USER_ID, API_ID, API_HASH, CACHE_SIZE_LIMIT, update_config
import aiohttp
...
async def get_command_usage(event):
    if event.sender_id == TELEGRAM_USER_ID:
        msg = await event.respond("Usage: /get <username>")
        USER_MESSAGES.append(msg.id)
...
async def get_command(event):
    ...
    stdout, stderr = run_script([username])
...
async def n_command_usage(event):
    if event.sender_id == TELEGRAM_USER_ID:
        msg = await event.respond("Usage: /n <number>")
        USER_MESSAGES.append(msg.id)

@client.on(events.NewMessage(pattern='/n (.+)'))
async def n_command(event):
    if event.sender_id != TELEGRAM_USER_ID:
        msg = await event.respond("Unauthorized access.")
        USER_MESSAGES.append(msg.id)
        logger.warning(f"Unauthorized access denied for {event.sender_id}.")
    ...
    stdout, stderr = run_script(['-n', str(number)])
    ...
    try:
        with open("subscriptions_list.txt", "r") as f:
            subscriptions = f.readlines()
        username = subscriptions[number - 1].strip()
        tag = f"#{username}"
    ...
    pinned_message = await event.respond(f"Started downloading media for subscription #{number} {tag}")
    TEXT_MESSAGES.append(pinned_message.id)
    pinned_message_id = pinned_message.id
    await client(UpdatePinnedMessageRequest(
        peer=event.chat_id,
...
async def setup_bot_commands():
    commands = [
        ...
        BotCommand(command='get', description='Download media from username'),
        BotCommand(command='n', description='Download media from subscription number'),
